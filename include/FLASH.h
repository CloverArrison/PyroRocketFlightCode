...
class myFlash{
public:
  myFlash(int cs);                  // Constructer takes in the chip select from config
...
private:
  ...
  void delFile(String fname);       // Deletes a file with a spesified name
  void writeDataFile(File file);    // Writes formated data to a file
};
